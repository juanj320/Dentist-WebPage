<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="style.css">
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans&display=swap" rel="stylesheet">
    <script src="https://kit.fontawesome.com/aa768af1b4.js" crossorigin="anonymous"></script>
    <style>
    @media screen and (max-width:1200px){
        img{
            width:100%;
        }
    }
    </style>
    <title>Ortodoncia</title>
</head>

<body>
    
<header class="maincontainer">

<div class="container1">

<a href="../index.php"><img src="logo.PNG" id="logo"></a>

</div>
    <nav class="navegacion">
        
        <ul class="menu">
            <li><a href="../index.php">Inicio</a></li>
            <li><a href="../tratamientos/tratamientos.php">Tratamientos</a>
                 <ul class=submenu>
                   <li><a href="../limpieza/limpieza.php">Limpieza</a></li>
                   <li><a href="ortodoncia.php">Ortodoncia</a>
                   <li><a href="../implantes/implantes.php">Implantes</a>
                   <li><a href="../endodoncia/endodoncia.php">Endodoncia</a>
                </ul>
            </li>
            <li><a href="../nosotros/nosotros.php">Nosotros</a></li>
            <li><a href="../agendar/agendar.php">Agenda tu cita</a></li>
        </ul>
    </nav>
</header>

<main>

<h1 class="t1">ORTODONCIA</h1>

<article class="article1">

<div id="img1"><img src="ort1.png"></div>

<p>
La ortodoncia corrige los defectos de la posición dental para mantener una boca perfecta y sana. 
En nuestra clínica dental SmileDent, somos especialistas en ortodoncia y realizamos diferentes tipos de
 tratamientos para adaptarnos a tus necesidades. Ventajas exclusivas en precios y financiación en 
 tratamientos dentales.

</p>

</article>


<h1 class="t2">ORTODONCIA INVISIBLE</h1>

<article class="article2">

<p id="text2">
La ortodoncia modifica la posición de las piezas dentales, pero a veces, los brackets convencionales pueden resultar
 molestos o no gustar estéticamente. Por ello, en la clininica SmileDent, sumamos a nuestros tratamientos la ortodoncia 
 invisible, una manera casi imperceptible de corregir tu sonrisa.Aprovecha las ventajas en precios y financiación
  que te ofrecemos.

</p>

<div id="img1"><img src="ort2.jpg"></div>

</article>

<button ><a href="../agendar/agendar.php">Agenda tu cita</a></button>

</main>

<footer>

<div class="inform">

<center><h3>Informacion</h3></center>

<ul>

    <li><a href="">Tratamiento de datos y privacidad</a></li>
    <li><a href="">terminos y condiciones</a></li>
    <li><a href="">Cookies</a></li>
    
</ul>


</div>

<div class="contactanos">

<center><h3>Contactanos</h3></center>

<ul>
    <li><a href="">Telefono:3157627</a></li>
    <li><a href="">Correo:Smiledentcontact@smiledent.com.co</a></li>
    <li><a href="">Direccion</a></li>

<ul>

</div>

<div class="Redes">

<center><h3>Redes Sociales</h3></center>

<a href=""><img src="data:image/svg+xml;base64," id="instagram" /></a>
<a href=""><img src="data:image/svg+xml;base64," id="facebook" /></a>
<a href=""><img src="data:image/svg+xml;base64," id="twiter" /></a>

</div>

</footer>

    
</body>
</html>