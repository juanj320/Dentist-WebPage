<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="style.css">
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans&display=swap" rel="stylesheet">
    <script src="https://kit.fontawesome.com/aa768af1b4.js" crossorigin="anonymous"></script>
    <title>Tratamientos</title>
</head>

<body>
    
<header class="maincontainer">

<div class="container1">

<a href="../index.php"><img src="logo.PNG" id="logo"></a>

</div>
    <nav class="navegacion">
        
        <ul class="menu">
        <li><a href="../index.php">Inicio</a></li>
            <li><a href="tratamientos.php">Tratamientos</a>
                 <ul class=submenu>
                   <li><a href="../limpieza/limpieza.php">Limpieza</a></li>
                   <li><a href="../ortodoncia/ortodoncia.php">Ortodoncia</a>
                   <li><a href="../implantes/implantes.php">Implantes</a>
                   <li><a href="../endodoncia/endodoncia.php">Endodoncia</a>
                </ul>
            </li>
            <li><a href="../nosotros/nosotros.php">Nosotros</a></li>
            <li><a href="../agendar/agendar.php">Agenda tu cita</a></li>
        </ul>
    </nav>
</header>

<main>

<div class="containerT">

<h1 class="t1">TRATAMIENTOS</h1>

<div class="containerimg">

<div id="img1"><img src="ort1.jpg" ></div>
<div id="img2"><img src="ort2.jpg" ></div>
<div id="img3"><img src="ort3.jpg" ></div>


</div>

<article>

<p>
Somos una entidad que presta servicios integrales de Ortodoncia,Odontología General, Odontología Estética,
Endodoncia, Implantes, con más de 20 años de experiencia en el eje cafetero.
 <br><br>



Obtuvimos la primera certificación ISO 9001 en Ortodoncia en Colombia. Contamos con un equipo 
altamente capacitado, cumpliendo con los requerimientos del Ministerio de Salud, lo que nos permite asegurar los
más exigentes niveles de bioseguridad y calidad en los procedimientos. <br><br>

 

Estamos comprometidos con la satisfacción de nuestros pacientes, ayudándolos a recuperar la confianza y la salud,
 porque para nosotros "Tú Sonrisa es lo más Importante".<br><br>


</p>


</article>

<div class="video">


<iframe width="560" height="315" src="https://www.youtube.com/embed/3epJ4naqZDE?controls=0&amp;finish=50" title="YouTube video player"
     frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
      allowfullscreen></iframe>





</div>

</div>


</main>

<footer>

<div class="inform">

<center><h3>Informacion</h3></center>

<ul>

    <li><a href="">Tratamiento de datos y privacidad</a></li>
    <li><a href="">terminos y condiciones</a></li>
    <li><a href="">Cookies</a></li>
    
</ul>


</div>

<div class="contactanos">

<center><h3>Contactanos</h3></center>

<ul>
    <li><a href="">Telefono:3157627</a></li>
    <li><a href="">Correo:Smiledentcontact@smiledent.com.co</a></li>
    <li><a href="">Direccion</a></li>

<ul>

</div>

<div class="Redes">

<center><h3>Redes Sociales</h3></center>

<a href=""><img src="data:image/svg+xml;base64," id="instagram" /></a>
<a href=""><img src="data:image/svg+xml;base64," id="facebook" /></a>
<a href=""><img src="data:image/svg+xml;base64," id="twiter" /></a>

</div>

</footer>

    
</body>
</html>